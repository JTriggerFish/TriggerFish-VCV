...
WireRackPreparedParameters PrepareWireRackParameters(
    const float sampleRate, const WireRackParameters &source) {
  ...
  const float low = Safe(source.minimumFrequencyHz, 900.f, 40.f,
                         .4f * sampleRate);
  const float high = Safe(source.maximumFrequencyHz, 15500.f, low + 1.f,
                          .48f * sampleRate);
  ...
  for (std::size_t mode = 0; mode < result.activeModeCount; ++mode) {
    const float position = (static_cast<float>(mode) + .5f +
                            .38f * random.Bipolar()) /
                           static_cast<float>(result.activeModeCount);
    const float frequency = low * std::pow(
        high / low, std::clamp(position, 0.f, 1.f));
    const float angle = TwoPi * frequency / sampleRate;
    const float modeDecay = decay * std::exp2(
        -2.f * decayTilt * std::clamp(position, 0.f, 1.f));
    const float phase = 3.14159265358979323846f * random.Bipolar();
    const float gain = std::exp2(-.35f * position) *
        (.75f + .5f * random.Uniform());
    result.cosine[mode] = std::cos(angle);
    result.sine[mode] = std::sin(angle);
    result.radius[mode] = std::exp(
        std::log(.001f) / (modeDecay * sampleRate));
  ...
  const float outputScale = 1.25f /
      std::sqrt(std::max(outputNormSquared, 1.e-12f));
  for (std::size_t mode = 0; mode < result.activeModeCount; ++mode)
    result.modeOutputGain[mode] *= outputScale;
  const float motionHz = Safe(source.motionHighpassHz, 140.f, 5.f,
                              .45f * sampleRate);
  result.motionCoefficient = 1.f - std::exp(-TwoPi * motionHz / sampleRate);
  const float attack = Safe(source.attackSeconds, .002f, .0001f, .05f);
...
float WireRack::Process(float bodyMotion) noexcept {
  ...
  motionLowpass_ += parameters_.motionCoefficient *
                    (bodyMotion - motionLowpass_);
  ...
  const float contact = std::clamp(
      parameters_.sensitivity * contactEnvelope_, 0.f, 1.f);
  ...
  const float inputGain = 1.f / std::sqrt(
      static_cast<float>(parameters_.activeModeCount));
  for (std::size_t mode = 0; mode < parameters_.activeModeCount; ++mode) {
    ...
    real_[mode] = parameters_.radius[mode] *
        (parameters_.cosine[mode] * priorReal -
         parameters_.sine[mode] * priorImaginary);
    imaginary_[mode] = parameters_.radius[mode] *
        (parameters_.sine[mode] * priorReal +
    ...
    crossEnergy += force *
        (parameters_.inputPhaseCosine[mode] * real_[mode] +
  ...
  return tfdsp::FiniteNormalOrZero(parameters_.outputLevel *
...
float WireRack::AvailableDriveScale(
    const float baseEnergy, const float crossEnergy,
    const float driveEnergy) const noexcept {
  const float proposed = baseEnergy + 2.f * crossEnergy + driveEnergy;
  if (proposed <= parameters_.maximumModalEnergy) return 1.f;
  if (driveEnergy <= 1.e-20f) return 0.f;
  const float discriminant = crossEnergy * crossEnergy + driveEnergy *
      std::max(0.f, parameters_.maximumModalEnergy - baseEnergy);
  return std::clamp((-crossEnergy + std::sqrt(discriminant)) / driveEnergy,
                    0.f, 1.f);
}
