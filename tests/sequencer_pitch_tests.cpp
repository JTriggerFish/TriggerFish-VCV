...
void keyRootsAndSettingOrder() {
  ...
  for (const std::string lane : {"chords i", "notes (1 3 5)",
                                  "chords Dm", "notes (D F A)"})
  ...
  for (const auto mode : {Mode{"major", 4, 7}, Mode{"dorian", 3, 7},
                          Mode{"phrygian", 3, 7}, Mode{"lydian", 4, 7},
                          Mode{"mixolydian", 4, 7}, Mode{"minor", 3, 7},
                          Mode{"locrian", 3, 6}}) {
    for (int root = 0; root < 12; ++root) {
      ...
      expectFirstChord("chords III\n" + scale + key,
                       {root + mode.third, root + mode.third + 4,
                        root + mode.third + 7});
    }
  }
  // An explicitly written tonic remains authoritative in either order.
  for (const std::string settings : {"key D\ntonic E@3\n",
                                      "tonic E@3\nkey D\n"})
...
void progressionsStayInRegister() {
  ...
  for (const auto progression : {
    ...
    for (const std::string style : {"basic", "rootless_3notes",
                                     "rootless_4notes"}) {
      for (const bool separateRhythm : {false, true}) {
        const auto compiled = tfseq::Compile(program(
            "subdiv 2n\nkey C\nvoicing " + style + "\nchords " +
            progression.chords + (separateRhythm ? "\nrhythm x x x" : "")));
        check(bool(compiled), "progression compiles: " + compiled.diagnostic.message);
        if (!compiled)
          continue;
        tfseq::Runtime runtime;
        runtime.setProgram(compiled.program.get());
        ...
        for (int step = 0; step < 300; ++step) {
          ...
          const int target = progression.roots[step % 3] +
                             (style == "basic" ? 6 : -2);
        ...
        check(allocations == 0, "playback performs no heap allocations: " + label);
      }
    }
  }
}
...
void registerChangesAndPlaybackState() {
  for (const std::string style : {"basic", "rootless_3notes", "rootless_4notes"}) {
    struct Case {
      const char *body;
      std::array<int, 3> roots;
    };
    for (const auto fixture : {
             Case{"chords C7@4 C7@6 C7@2", {0, 24, -24}},
             Case{"chords C7 C7' C7,", {0, 12, -12}},
             Case{"octave 4 6 2\nchords C7", {0, 24, -24}},
             Case{"chords C7 |> octave 2", {24, 24, 24}}}) {
      auto compiled = tfseq::Compile(program("voicing " + style + "\n" + fixture.body));
      ...
      for (int step = 0; step < 30; ++step) {
        ...
        if (events.count == 0)
          continue;
        const float midpoint = 6.f * (events.events[0].pitchVolts +
            events.events[events.count - 1].pitchVolts);
        const int target = fixture.roots[step % 3] + (style == "basic" ? 6 : -2);
        followsRegister &= std::abs(midpoint - target) <= 6.001f;
      }
      check(followsRegister, "written register changes override prior context: " + style);
    }
  }
  ...
  if (slash) {
    ...
    check(events.count == 3 &&
          ...
          "a slash bass keeps its explicit register and removes the duplicate tone");
  }
}
} // namespace
