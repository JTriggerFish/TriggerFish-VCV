...
export const hasPairedRing = (layout, rate) => Math.round(layout) === 3 && rate > 0;
...
export function mountPacketLayout(parent, {read, set, reset}) {
  ...
  row.innerHTML = '<span>Ring character</span><select aria-label="Ring character"><option value="3">Paired ring</option><option value="0">Scattered</option><option value="1">Even coverage</option><option value="2">Beating doublets</option></select>';
