...
const descriptors = [
  ...
  ...Array.from({ length: 12 }, (_, index) => ({
    key: `wash_frequency_${index}`, defaultValue: 100 * (index + 1),
    minimum: 40, maximum: 22000, scale: "logarithmic", index: index + 2,
  })),
  ...Array.from({ length: 12 }, (_, index) => ({
    key: `wash_level_${index}`, defaultValue: 0,
...
assert.equal(values(0).wash_frequency_5, 600 * .42);
assert.equal(values(1).wash_frequency_5, 600 * 1.45);
assert.equal(values(0).wash_level_0, 5);
assert.equal(values(1).wash_level_5, 7);
